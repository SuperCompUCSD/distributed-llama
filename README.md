...
Enabling dotprod at the default 2.4GHz clock frequency caused the head node (rock0) to crash with a hard hardware reset (no kernel panic or error message, just an immediate reboot). Through systematic frequency testing, we identified that rock0's power delievery on A76 core rail cannot sustain dotprod workloads above 1992MHz, while worker nodes remained stable at all frequencies. So we capped the root node CPU frequency to 1992MHz.  
